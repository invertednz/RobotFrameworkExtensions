...
class TestRunnerPlugin(Plugin):
    ...
    def _register_actions(self):
        ...
        pause_action_info = ActionInfo("Tools", "Pause Test Run", self.OnPause, None,
                                      "F9", ImageProvider().TOOLBAR_PAUSE, "Pause a running test",position=12)
        self._pause_action = self.register_action(pause_action_info)
        
        continue_action_info = ActionInfo("Tools", "Continue Test Run", self.OnContinue, None,
                                      "CtrlCmd-F9", ImageProvider().TOOLBAR_CONTINUE, "Continue a running test",position=13)
        self._continue_action = self.register_action(continue_action_info)
        
        next_action_info = ActionInfo("Tools", "Run Next Step", self.OnStepNext, None,
                                      "F10", ImageProvider().TOOLBAR_NEXT, "Run Next Step",position=14)
        self._next_action = self.register_action(next_action_info)
        
        stepover_action_info = ActionInfo("Tools", "Step Over", self.OnStepOver, None,
                                      "CtrlCmd-F10", ImageProvider().TOOLBAR_NEXT, "Step Over",position=15)
        self._stepover_action = self.register_action(stepover_action_info)
    ...
    def _build_notebook_tab(self):
        ...
        self._progress_bar = ProgressBar(self._right_panel, self)
    ...
    def _handle_start_keyword(self, args):
        '''attrs = args[0] + args[1]['args']'''
        attrs =  ' '.join(args[1]['args'])
        attrs = args[0] + ' ' + attrs
        '''self._append_to_message_log('Ending test:   %s\n' % attrs)'''
        '''print '%s' % (args[1]['args'])'''
        self._progress_bar.set_current_keyword(attrs)
    ...
    def _set_running(self):
        ...
        self._pause_action.enable()
        self._continue_action.disable()
        self._next_action.disable()
        self._stepover_action.disable()
    ...
    def _set_paused(self):
        ...
        self._stop_action.enable()
        self._pause_action.enable()
        self._continue_action.enable()
        self._next_action.enable()
        self._stepover_action.enable()
...
class ProgressBar(wx.Panel):
    ...
    def __init__(self, parent, testRunnerPlugin):
        ...
        self.testRunnerPlugin = testRunnerPlugin
    ...
    def _get_current_keyword_text(self):
        if not self._current_keywords:
            return ''
        #return '     current keyword: '+' -> '.join(self._current_keywords)
        ...
        breakPoint = "false"
        data = ""
        lastKeyword = 'Next Keyword: '+self._fix_size(' -> '.join(self._current_keywords), 125)
        if lastKeyword!=self.prevKeyword:
            self.testRunnerPlugin._output("COMPLETED %s\n" % (str(datetime.datetime.now())))
            ...
            self.testRunnerPlugin._output("%s\n" % lastKeyword)
        self.prevKeyword=lastKeyword
        return '     current keyword: '+self._fix_size(' -> '.join(self._current_keywords), 200)
